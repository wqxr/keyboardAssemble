...
class WebSocketUtil {
    ...
    */
        ...
        return new Promise((resolve, reject) => {
            ...
            this.websocket.addEventListener('open', function (event) {
                ...
                self.callbackArray.forEach((item) => {
                   ...
                   self.mainWindow.webContents.send(MSG_TYPE.SEND_TO_PAGE, {msgType:"connect",data:null});
            ...
            this.websocket.addEventListener('close', function (event) {
                ...
                if( isClose ) {
                    self.mainWindow.webContents.send(MSG_TYPE.SEND_TO_PAGE, {msgType:"closeConnect",data:0});
                    isClose = false;
                }
                self.reConnect();
            });
    ...
    async handleMessageType(message) {
        ...
        // switch(msgType){
        //     case MSG_TYPE.FEED_MATERIAL:
        //         todoHandler= await this.handler.productionDetailHandler.run(message.data);
        //         break;
        //     case MSG_TYPE.OUT_MATERIAL:
        //         todoHandler= await this.handler.pcbTotalHandler.run(message.data);
        //         break;
        // }
        if (msgType == "outmaterial") {
            todoHandler = await this.handler.pcbTotalHandler.run(message.data);
            console.info("wq",message.data);
        } else if (msgType == "feederinfomation") {
            todoHandler = await this.handler.productionDetailHandler.run(message.data);
    ...
    writeLog(data) {
        try {
            ...
            if (message.msgType === "sendMsg" && undefined !== message.data["error"]) {
                excelLogUtil.addErrorLogRecord(message.data.ID, message.data.error);
                //logger.errorLog( message.data );
            } else if (message.msgType === "sendMsg" && undefined !== message.data["operate"]) {
                excelLogUtil.addOperateLogRecord(message.data.operate);
            } else if (message.msgType === "feederinfomation") {
                //excelLogUtil.addProductDetail(message.data.code,message.data)
            }
            //报警信息
            else if( message.msgType === "warn" ){
                let analyiseMessage = message.data.msg.split(",");
                let errorCode  = analyiseMessage[0];
                let type =  analyiseMessage[1]
                let detailErrorMessage = analyiseMessage[2];
                excelLogUtil.addErrorLogRecord(errorCode,type,detailErrorMessage);
            }else {
                //logger.message(data);
            }
        } catch (error) {
